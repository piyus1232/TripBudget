import { getRoundTripTrains } from "./train.controller.js";
...
// Main controller to get the cheapest round-trip trains with time gap logic
const getCheapestRoundTripTrains = async (req, res) => {
  try {
    const { source, destination, startDate, returnDate, classCodes = ['SL'], forceRefresh = false } = req.body;
    ...
    const isSameDayFinal = isSameDay || upstreamIsSameDay; // Use upstream if available

    // Debug: Log input data
    console.log("Filtered Outbound Trains:", JSON.stringify(filteredOutTrains));
    console.log("Filtered Return Trains:", JSON.stringify(filteredReturnTrains));
    ...
    if (cheapestOutTrain && validReturnTrains.length > 0) {
      ...
      if (isSameDayFinal) {
        filteredReturnTrainsWithGap = sortedReturnTrains.filter(train => isValidReturnTimeGap(cheapestOutTrain.train_base.from_time, train.train_base.from_time));
        if (filteredReturnTrainsWithGap.length === 0) {
          ...
          filteredReturnTrainsWithGap = [sortedReturnTrains[0]]; // Fallback to cheapest
        }
      }
      ...
      if (secondCheapestOutTrain && validReturnTrains.length > 1) {
        let filteredSecondReturnTrainsWithGap = sortedReturnTrains.filter(train => train.train_base.train_no !== cheapestReturnTrain?.train_base.train_no); // Avoid duplication
        if (isSameDayFinal) {
          filteredSecondReturnTrainsWithGap = filteredSecondReturnTrainsWithGap.filter(train => {
            ...
            if (!gap) console.log(`Train ${train.train_base.train_no} at ${train.train_base.from_time} does not meet 6-7 hour gap from ${secondCheapestOutTrain.train_base.from_time}`);
            return gap;
          });
          if (filteredSecondReturnTrainsWithGap.length === 0) {
            console.log("No return trains found within 6-7 hour gap for second-cheapest, using next available with gap check");
            ...
            if (filteredSecondReturnTrainsWithGap.length === 0) {
              console.log("No valid second-cheapest return train found with 6-7 hour gap, using next cheapest available");
              filteredSecondReturnTrainsWithGap = [sortedReturnTrains.find(train => train.train_base.train_no !== cheapestReturnTrain?.train_base.train_no) || sortedReturnTrains[1] || sortedReturnTrains[0]];
            }
          }
        }
        secondCheapestReturnTrain = filteredSecondReturnTrainsWithGap[0];
      } else {
        secondCheapestReturnTrain = sortedReturnTrains.length > 1 && sortedReturnTrains[1]?.train_base.train_no !== cheapestReturnTrain?.train_base.train_no ? sortedReturnTrains[1] : null;
      }
      ...
      if (cheapestReturnTrain && normalizeTrainName(cheapestReturnTrain.train_base?.train_name || '') === outTrainName) {
        const differentTrains = filteredReturnTrainsWithGap.filter(train => normalizeTrainName(train.train_base?.train_name || '') !== outTrainName);
      ...
      if (secondCheapestReturnTrain && normalizeTrainName(secondCheapestReturnTrain.train_base?.train_name || '') === outTrainName) {
        const differentTrains = filteredSecondReturnTrainsWithGap.filter(train => normalizeTrainName(train.train_base?.train_name || '') !== outTrainName);
        secondCheapestReturnTrain = differentTrains[0] || secondCheapestReturnTrain;
      }
    }

    // Construct response with fares for all requested classes
    const response = {
      success: true,
    ...
    console.error("Error in getCheapestRoundTripTrains:", error);
...
export { getCheapestRoundTripTrains };