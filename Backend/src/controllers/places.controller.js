...
const getPlaces = async (destination) => {
  ...
  const rapidGooglePlacesKey = googlePlacesRapidApiKey();
  if (!rapidGooglePlacesKey) {
    throw new ApiError(
      500,
      'GOOGLE_PLACES_RAPIDAPI_KEY is not set (RapidAPI key for google-map-places.p.rapidapi.com)'
    );
  }

  // Step 1: Geocoding (RapidAPI Google Map Places proxy)
  ...
  try {
    const geoResponse = await axios.get(
      'https://google-map-places.p.rapidapi.com/maps/api/geocode/json',
      {
        ...
        headers: {
          'X-RapidAPI-Key': rapidGooglePlacesKey,
